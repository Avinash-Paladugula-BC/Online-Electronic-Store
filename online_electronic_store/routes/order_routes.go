...
func OrderRoutes(r *gin.Engine, db *gorm.DB) {
	...
	r.GET("/orders", middleware.AuthMiddleware(), orderTable.GetOrders) 
	r.POST("/orders/create", middleware.AuthMiddleware(), orderTable.MakeOrder)
	r.PUT("/orders/:order_id", middleware.AuthMiddleware(), orderTable.UpdateOrder)
}